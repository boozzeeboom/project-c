...
## Итерация от 2026-07-18 (v1.12 — проверка на дубликат)

**Задача:** При создании корабля скрипт молча перезаписывал существующий префаб. Нужно предупреждать пользователя и не перезаписывать.

**Коммит:** `c7a5dbb` — T-SHIP08: проверка на дубликат имени корабля перед созданием префаба

**Изменения:**
- `Assets/_Project/Editor/ShipPresetCreator.cs` — в `OnGUI` добавлена проверка `AssetDatabase.LoadAssetAtPath<GameObject>(prefabPath)` перед вызовом `CreateShipPreset`
- Если префаб существует — `EditorUtility.DisplayDialog("Предупреждение", …)` и `return` без закрытия окна (можно ввести другое имя)
