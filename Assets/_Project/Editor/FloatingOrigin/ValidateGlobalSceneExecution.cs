...
namespace ProjectC.EditorTools.FloatingOrigin
{
    ...
    public static class ValidateGlobalSceneExecution
    {
        ...
        public static Report Run()
        {
            ...
            Check("Scene-owned gameplay ownership requires world network identity", () => { var f = new GlobalSceneOwnershipFacts { Ownership = GlobalSceneOwnership.SceneOwnedNetworkGameplay, NetworkObject = true, ScenePath = "Assets/_Project/Scenes/World/WorldScene_0_0.unity" }; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) == null); f.HasRigidbody = true; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) != null); });
            Check("Persistent Bootstrap service may remain in DDOL without network identity", () => { var f = new GlobalSceneOwnershipFacts { Ownership = GlobalSceneOwnership.PersistentBootstrapService, NetworkObject = false, ScenePath = "Assets/_Project/Scenes/BootstrapScene.unity" }; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) == null); f.ScenePath = "Assets/_Project/Scenes/World/WorldScene_0_0.unity"; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) != null); });
            Check("Ship ownership requires a Rigidbody and cannot enter static gameplay contract", () => { var f = new GlobalSceneOwnershipFacts { Ownership = GlobalSceneOwnership.ShipOrRigidbodyRoot, NetworkObject = true, HasRigidbody = true, ScenePath = "Assets/_Project/Scenes/World/WorldScene_0_0.unity" }; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) == null); f.HasRigidbody = false; Require(GlobalSceneExecutionPolicy.ValidateOwnership(f) != null); });
            Check("Catalog ownership cannot silently accept unknown source category", () => Require(GlobalSceneExecutionPolicy.ValidateOwnership(new GlobalSceneOwnershipFacts { Ownership = GlobalSceneOwnership.Unspecified }) != null));
        ...
        private static void Fixture(out GlobalSceneLifecycleLedger ledger, out GlobalSceneLoadTicket ticket, out string root, out string child)
        {
            ...
            var data = new GlobalSceneCatalogData { expectedSceneGuids = new[] { guid }, scenes = new[] { new GlobalSceneSource { sceneGuid = guid, assetPath = "Assets/_Project/Scenes/World/WorldScene_0_0.unity", saved = true, inspectedComplete = true, dependencyHash = new string('b', 32), observations = new[] {
                ...
                new GlobalSceneEntry { sceneGuid = guid, sourceId = root, treatment = GlobalSceneTreatment.SceneNetworkObject, ownership = GlobalSceneOwnership.SceneOwnedNetworkGameplay, reviewNote = "pure fixture" },
                new GlobalSceneEntry { sceneGuid = guid, sourceId = child, parentSourceId = root, treatment = GlobalSceneTreatment.SceneNetworkObject, ownership = GlobalSceneOwnership.SceneOwnedNetworkGameplay, reviewNote = "pure fixture" } } };
