...
## Итерация 11 — 2026-08-24

**Задача:** T-UI09 — откат небезопасного глобального rebuild локализации после реализации contextual interaction hint
**Откат:** `be7ba8f13cdada4b4c1f89daf204c5561e11eb94`
**Документ:** `06_POSTMORTEM_T-UI09_Localization_Blast_Radius.md`

**Наблюдение:**
- микрофича требовала только два UI-ключа и не должна была менять глобальное состояние локализации;
- editor-команда `Rebuild Tables (SharedData fix)` перестраивала сразу Static/System/UI/Dialogue;
- UI rebuild использовал неполный `UIKeyMap` вместо объединения всех CSV и runtime-ключей;
- в результате были потеряны/рассинхронизированы записи UI, Static и System, включая orphan-записи с несовпадающими SharedData ID;
- зелёная C#-компиляция не проверяла целостность сериализованных таблиц.

**Статус:** изменения после целевого коммита удалены откатом; наблюдение и причинная цепочка зафиксированы в документации. Play Mode и screenshots не выполнялись согласно рабочему ограничению проекта.
