...
## Ресерч порядка спавна (гипотеза пользователя подтверждена) + отложенная регистрация

Документ: `04_SPAWN_VS_RESTORE_ORDER.md`. Таймлайн host-старта: спавн кораблей t≈0
(сразу варнинги `CombatServer/MetaRequirementRegistry/GatheringServer/InventoryWorld
== null` — корабли раньше серверов), restore-телепорт t≈3.5 с
(`restoreDelaySec`, `ShipPositionServer.cs:45,161-217`) → drift-Add ×20.
Свежие счётчики (`spawn=40 drift=40` за 2 сессии) + нулевые `fo-*` это подтверждают.

Фикс (коммит ниже): `ShipDeckNav` не встаёт в очередь регистрации, пока
`ShipPositionServer.RestoreCompleted == false` (дешёвая проверка в `LateUpdate`).
Страховки: `Instance==null` → как раньше; таймаут 60 с → принудительно.
Клиентский путь не тронут. Ожидание: spawn-Add на финальной позиции, `drift≈0`,
−20 Add и −20 штормов за сессию. Цена: экипажная навигация позже на ~5 с.
Проверка: Console 0 errors.
Протокол: новая сессия → `DumpStats()` (spawn≈20, drift≈0) + лог варнингов.

## Лог `оптим_сент_5.txt` (397 варнингов) + живые счётчики

- Агент-варнинги: 675 → 314 (−53%), Add-стеки 636 → 275 (−57%), всего 758 → 397 (−48%).
- Форма изменилась: стартового залпа больше нет, остаток — ровная капель всю сессию
  (блоки 85–397, ~50 на каждые 50 варнингов; не-агентных поздних нет).
- Живые счётчики: `spawn=20 drift=0` — отложенная регистрация + кулдаун убрали
  парные Add полностью (в текущем домене).
- Нестыковка 275 vs 20/0: лог копит несколько сессий (консоль не чистилась),
  счётчики — только текущий домен. Протокол дальше: **Clear консоли перед Play**.
- Добавлен счётчик `rebase-resets` (`NotifyWorldRebased`): отличит drift от полётов
  (>2500 м) от drift после сдвигов. Проверка: Console 0 errors, `DumpStats()` жив.
- Следующий замер (чистая сессия): `DumpStats()` + число «Failed to create agent».
  Если `drift ≈ rebase-resets × корабли` — чиним сдвиг (`_navFrameOrigin` stale),
  если drift без rebase — дрейф от полётов (поднять separation / троттлить).

## Лог `оптим_сент_6.txt` (целиком, 3321 строка, 415 типов) — вердикт

- Агент-варнинги 314 / Add-стеки 275 — **побайтово те же числа, что в логе 5**.
  Новых сессий с предупреждениями не было: это та же накопленная история консоли.
- Живые счётчики при этом: `spawn=20 drift=0 fo-* = 0 rebase-resets=1`.
  Текущий домен здоров: был 1 rebase, и он НЕ дал drift-Add
  (сдвиг случился до регистрации палуб — отложенная регистрация заодно покрыла
  и ранние сдвиги; либо корабли ехали корнями вместе с мешем).
- Лог 6 впервые включает обычные `[Log]` (569 шт.): новый фон —
  `MeziyModuleActivator`, `ShipController meziy NOT found` ×22, `KeyRodInstance`
  сейвы ×22, `CombatServer Registered/RecoverExistingEntities`, `ShipTelemetry`
  ×19, `ShipFuelSystem`. Это разовые стартовые строки, не спам, но кандидаты
  на downgrade при следующей волне (пока не трогаем — сначала чистый замер).
...
## Пункт (а): парковка агентов при снятом меше (выполнена, замер — за пользователем)

Диагноз из лога 7 (одна сессия): 789 агент-варнингов при ~33 Add → ~24 шт./Add.
Механизм: прокси никогда не дизейблился — при снятом меше (окно между Unregister
и re-Add, пре-регистрация, сдвиги) висел включённым off-mesh, и каждый чужой Add
реасессил всех голодающих с варнингом. Подтверждено кодом (`NpcBrain`: enable
только в `EnsureProxy`-создании, disable — нигде).
Фикс (`NpcBrain.cs`, 3 точки, семантика остальных проверок `enabled` не менялась):
1. Вход в deck-режим при неготовом меше — `_proxyAgent.enabled = false`.
2. `EnsureProxy` reuse-ветка (вход только при готовом меше) — re-enable.
3. `DriveDeckNav` — ранний return при запаркованном (не дёргать Warp).
Проверка: Console 0 errors. Ожидание: агент-варнинги 789 → десятки (только честные
мимо-меша), `CallLogCallback`/`ExtractStackTrace` вниз пропорционально.
Ручная проверка (пользователь, NOT RUN): экипаж ходит по палубе летящего корабля,
догоняет/останавливается штатно; после F8 — без залипания.
Протокол: чистая сессия (Clear!) → число «Failed to create agent».

## Ложная тревога: «пропал экипаж» (2026-09-25) + ловушка замеров

Наблюдение пользователя: кроме капитанов не видно простых NPC с цикличным спавном.
Расследование вживую через MCP:
1. Манифесты всех 20 спавнеров содержат ровно по 1 члену — капитана
   (`torrens_captain_01` и т.д.). «Простых» в экипажах нет по данным, не по коду.
2. 20 `onMesh` ранее — это и были капитаны. Мои правки (парковка, дефер) тел не
   трогают: прокси невидимы (`HideAndDontSave`), смерть тел не сносит
   (`EnterDead` — только анимация), спавнер сам не деспавнит.
3. Поймана методологическая ловушка: в проекте выключены Domain Reload и Scene Reload
   (`enterPlayModeOptions`), `isPlaying=false` на момент чтения — все «живые» чтения
   после стопа показывают мусор пост-сессии (деспавн экипажа при шатдауне — штатно,
   спавнеры `enabled=false` — штатно для не-сервера, статики — frozen).
   Правило: живые выводы только при подтверждённом `Application.isPlaying`.
4. Цикличные ambient-NPC — это `NpcSpawner` (23 шт., гейт `activationRadius` +
   server-only). Их видимость проверять только во время Play рядом с точкой спавна.
Фикс инструмента: `ShipDeckNav.ResetStats()` + вызов из
`ShipPositionServer.PrepareForServerStart` — счётчики больше не текут между сессиями.
Проверка: Console 0 errors.
