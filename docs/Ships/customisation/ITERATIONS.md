...
## Итерация от 2026-09-08 (T-ENG02)

**Задача:** Исправить поворот кастомного FBX-визуала двигателя и синхронизировать отдельный ротор с поворотом двигателя.

**Изменения:**
- `Assets/_Project/Scripts/Ship/Engine/EngineThrusterVisual.cs` — базовые позы двигателя и ротора вычисляются в общем пространстве `Slot_Engine`.
- `EngineThrusterVisual` применяет одинаковое отклонение вокруг `RotationAnchor` к `MOTOR_GameReady` и отдельному sibling `ROTOR_GameReady`.
- Вращение ротора по собственной оси сохранено отдельно от поворота двигателя.
- Поддержаны вложенные FBX-корни, разные parent-трансформы и импортированные смещения.

**Проверки:**
- Компиляция Unity: ошибок нет ✅
- Play Mode: не выполнялся; требуется ручная проверка пользователем.
