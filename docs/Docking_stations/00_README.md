...
- ⚠️ **`DockPadVisualMarker`** — Quad-метки создаются, но **не меняют цвет при изменении `IsShipInside`** (текущая реализация использует `Physics.OverlapSphere`, который не находит корабль из-за слоёв/расстояния). **Требует переработки** (отдельный тикет `T-DOCK-14`).
