...
## 🔴 ПРАВИЛО: ОБЪЕКТЫ ЧЕРЕЗ СКРИПТЫ — ВСЕГДА УМЕНЬШЕННЫЙ МАСШТАБ
...
**Проблема (сессия A5 — FarmPlatform):**
- Платформа 40×20 → здания 6×4×4 → террасы 40×20 → всё «сбилось в кучу»
- При ручном увеличении масштаба ×5 фермы стали выглядеть нормально
- Координаты расположения правильные, проблема ТОЛЬКО в размере объектов

**Причина:** При скриптовой генерации мешей мы используем «удобные» числа (40, 20, 6), но они слишком малы для игрового мира, где горы — 300-600 units в радиусе.

**Решение — коэффициент ×5 для всех скриптовых объектов:**
...
**ВАЖНО:** Координаты расположения (X, Y, Z) — ПРАВИЛЬНЫЕ, их НЕ менять! Масштабируются ТОЛЬКО размеры самих объектов.

**Как применять:**
1. При создании любого скриптового объекта — умножать все размеры ×5
2. Или добавить `public float scaleMultiplier = 5f;` и применять ко всем размерам
3. Проверяй визуально: объект должен быть сопоставим с горами (радиус 300-600 units)

---

## ⚠️ ПУТАНИЦА МАСШТАБОВ — ИСТОЧНИК ОШИБОК

> **В проекте существует критическая путаница между тремя системами координат. Это вызывает постоянные ошибки при создании скриптов.**

### Три системы координат в проекте:

| Система | XZ масштаб | Y масштаб | Где используется |
|---------|-----------|-----------|-----------------|
| **1. Межгородские расстояния** | ×50 от оригинала → радиус ~350,000 | 1:100 (scaled) | Координаты 5 городов, 29 пиков |
| **2. Высоты объектов** | — | МЕТРЫ (не scaled!) | Y ферм (2500-4500), Y завесы (1200) |
| **3. Размеры скриптовых объектов** | ×5 от «удобных» чисел | ×5 от «удобных» чисел | FarmPlatform, TerraceBuilder, здания |

### История путаницы:

1. **Оригинальный дизайн** заявлял масштаб 1:2000 для расстояний → радиус мира ~7,000 units
2. **Проблема:** при радиусе 7,000 игрок стоя на Эвересте ВИДЕЛ площадку Кибо — нереалистично!
3. **Решение (сессия SCALE):** все XZ координаты ×50 → радиус мира ~350,000 units
4. **Новая проблема:** скриптовые объекты (фермы, здания) остались в старом масштабе → «сбились в кучу»
5. **Решение (сессия A5):** все размеры скриптовых объектов ×5

### Как НЕ ошибиться:

| Что создаёшь | Какой масштаб | Пример |
|-------------|--------------|--------|
| Координаты города/пика | ×50 от старого дизайна | Эверест→Аконкагуа = 234,000 units |
| Высота объекта (Y фермы) | МЕТРЫ, НЕ scaled | Y = 4,200 (не 42.00!) |
| Размер объекта (платформа, здание) | ×5 от «удобных» чисел | Платформа 200×100 (не 40×20!) |
| Радиус горного меша | 15,000-100,000 units | Эверест baseRadius = 30,000 |

### Файлы с актуальными координатами:

| Файл | Статус | Масштаб |
|------|--------|---------|
| `docs/world/WorldLandscape_Design.md` | ✅ Актуален (v2) | XZ ×50, Y города scaled 1:100, Y ферм метры |
| `docs/world/Landscape_TechnicalDesign.md` | ⚠️ Частично устарел | Содержит старые XZ координаты |
| `Assets/_Project/Scripts/Editor/FarmPlacementEditor.cs` | ✅ Актуален | XZ ×50, Y ферм метры |
| `Assets/_Project/Scripts/World/Core/FarmData.cs` | ✅ Актуален | Размеры ×5 |
