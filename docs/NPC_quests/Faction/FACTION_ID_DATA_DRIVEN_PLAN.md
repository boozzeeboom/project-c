...
- Изменения пяти authoring-ассетов сохранены отдельным asset-коммитом `8a6a827a`, потому что каталоги NPC/Quest исключены из обычного `git add`-пути проекта.

Статус: **закрыт по реализации**. Основной коммит: `8988a3a0`; asset-коммит миграции: `8a6a827a`. Runtime-потребители переключены на references на этапе 5.
...
Статус: **закрыт по реализации**; финальная editor/runtime/persistence/CSV проверка пользователем остаётся отдельной проверкой. Коммит: `a5a97889`.
