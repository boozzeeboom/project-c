...
# (Phase 2) recipes — отдельно, не импортируется этим тулом.
```

**Acceptance:**
- [ ] Preview: 1+1+1+1 = 4 rows, 0 errors (НО: `Железная руда` и `Древесина` должны быть
  в inventory — иначе ошибка cross-validate).
- [ ] Если `Железная руда` и `Древесина` ещё не импортированы — добавить строки в inventory.
- [ ] Import → "Created 1 ItemData, 1 TradeItem, 1 MarketItem, 1 ExchangeRate".
- [ ] Проверить файлы:
...
> Рецепт `Recipe_TestPickaxe` (если нужен) импортируется отдельно — Phase 2.
...
> ⚠️ **Phase 2:** блок `recipes` не входит в MVP. Эти 5 рецептов
> импортируются отдельным `CraftingCsvImporter` (см. `Crafting_system/` roadmap).
> Для справки — текущий набор:
> ```
> Recipe_IronIngot, Recipe_CopperIngot, Recipe_ShipKeyLight, Recipe_ShipKeyMedium, Recipe_ShipKeyHeavy
> ```

**Acceptance:**
- [ ] CSV содержит 47+5+8+4 = **64 строк данных** (без заголовков/комментариев).
...
6. `Tools → ProjectC → Resources → CSV Import/Export` → Browse → Preview → Import.

7. Console: "Created 1 ItemData (Медный камень, id 48), Created 1 TradeItem
   (resource_copper_stone_box), Created 2 MarketItem, Created 1 ExchangeRate.
   Total 5 changes. 0 errors."
...
9. (Опц.) Создать PickupItem в `WorldScene_0_0` с `itemData = Медный камень`.

10. Play Mode → подобрать PickupItem → Inventory → Exchanger tab → Упаковать 100 шт →
    +1 ящик на складе.

> **Рецепт для крафта «Слиток медного камня»:** импортируется в Phase 2
> (отдельный `CraftingCsvImporter`, см. `Crafting_system/` roadmap).
...
# (Phase 2) recipes — для справки, импортируется отдельно.
# Если оставить блок recipes в CSV — парсер распознает секцию, importer выдаст warning "skipped".
```
...
- Диалог: "Skipped 8 rows. 0 created, 0 updated. 8 errors."
...
| `CraftingStationConfig._allowedRecipes[]` | 3 (CraftingTable) | 3 | OK (Phase 2: Crafting import) |
...
# 2. Открыть dump.csv в Excel — 4 секции (без recipes), ~55 строк
...
| ~~`string recipeName`~~ | ~~`RecipeData.assetPath`~~ | ~~"Recipe_IronIngot"~~ | **Phase 2 (out of scope MVP):** отдельный `CraftingCsvImporter` |
