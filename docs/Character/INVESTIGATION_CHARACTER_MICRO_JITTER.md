...
> **Актуализация 2026-09-08 — T-JITTER17:** пользователь подтвердил отсутствие джиттера у глобального origin и появление на 30000+. Старые FloatingOrigin/FloatingOriginMP не используются. Анализ действующей Bootstrap, ресерч Asset Store и ограниченный план записаны в `docs/Character/INVESTIGATION_CHARACTER_MICRO_JITTER_SOLUTIONS_RESEARCH.md`. Масштабная миграция мира из §9.5 **отложена**: сначала проверяется возможность исправления только визуального слоя персонажа без изменения сети, физики и сохранений. Готовый plug-and-play фикс для нашего NGO/Humanoid случая пока не подтверждён; новый эксперимент не выполнен. Измерения и отрицательные результаты ниже сохраняются как история, не как список изменений для повторного применения.
