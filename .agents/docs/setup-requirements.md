...
| **QWENCODE** | AI agent CLI | `npm install -g QWENCODE-ai` |
...
- Hooks use `bash .QWENCODE/hooks/[name].sh` — this works on Windows because
  QWENCODE invokes commands through a shell that can find `bash.exe`
...
QWENCODE works with any editor, but the template is optimized for:
- **VS Code** with the QWENCODE extension
- **Cursor** (QWENCODE compatible)
- Terminal-based QWENCODE CLI
