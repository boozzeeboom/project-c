...
## 6. Публикация пилотного префаба

На момент этапа 06B `.gitignore:153` содержал `*.prefab`, поэтому пилотный префаб не попадал в коммит и существовал только локально — как и BootstrapScene под правилом `*.unity`.

**Решено в T-FO06C:** по указанию пользователя добавлено узкое исключение только для пилотной папки, вместо снятия общего правила. Подробности — `06C_PILOT_PREFAB_TRACKING.md`.
