...
## Итерация от 2026-09-18 (T-SHIP-FIX05)

**Задача:** P0 — двойной `dt` у мезия-тяги (`dt^2`). Статус: ИСПРАВЛЕНО (код, 1 строка).

**Перепроверка (до фикса — ПОДТВЕРЖДЕНО):** `:2086` — единственный `AddForce(* dt, ForceMode.Force)`
в файле (thrust/gravity/lift/wind/торки мезии — все без `dt`); `ForceMode.Force` интегрирует сам.
После фикса `* dt, ForceMode` в файле — 0 (поиск по диску). Сила вырастет ×50
(`meziyForce` 500–1000 в ассетах) — доводка силы только через SO (баланс, не этот тикет).

**Изменения (`ShipController.cs`, +3/−1):** убран `* dt` + коммент-причина.
