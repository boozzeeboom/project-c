...
### 3.2 Базовая торговля (Недели 9-11)
...
5. **Серверная валидация:**
   ...
   - Лимит транзакций, античит

### 3.3 Контракты НП (Недели 11-13)
1. **ContractSystem:**
...
### 3.4 Динамическая экономика (Недели 13-14)
1. **Tick-система:** ✅ СЕССИЯ 6 ЗАВЕРШЕНА
   ...
   - ✅ Затухание спроса/предложения — ELASTIC "КАЧЕЛИ" (0.846/тик, возврат из пика за ~80 мин)
...
3. **Серверная валидация инвентаря (anti-cheat)** ✅
4. **⏳ Сохранение состояния рынка в JSON** (отложено — добавить при реализации persistence)
   - Сохранение: demand/supply факторы, active events, tick-таймер
   - Восстановление при перезагрузке сервера
   - Формат: `Application.persistentDataPath/market_save.json`

**Результат:** Сохранение прогресса между сессиями, работающая прокачка, **полная система торговли с динамической экономикой, контрактами НП и долговой системой**.
...
2. **Визуальные улучшения:**
   ...
   - **Визуал торговли:** 3D модели сундуков грузов, NPC-торговцы, торговые посты
...
4. **P2P торговля между игроками:**
   - UI обмена, серверная валидация, налог 5%

**Результат:** Вертикальный срез игры с основным циклом геймплея, **работающая торговля «Дальнобойщики над облаками»**.
