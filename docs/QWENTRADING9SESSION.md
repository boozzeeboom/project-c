...
backup: backup/session5-complete-server-trade  ← текущая (перед Сессией 6)
backup: backup/session5-server-trade-working   ← до фиксов Сессии 5
```
...
- [ ] Затухание elastic работает: demand/supply уменьшаются ×0.846 за тик
- [ ] 4 NPC-трейдера перемещают товары между локациями
- [ ] Пассивная регенерация стока работает (+2% за тик)
- [ ] Массовая скупка мезия → цена выросла → через 3-4 тика упала (качели)
- [ ] Событие "Мезиевая лихорадка" → demand ×1.4 для мезия
- [ ] Delta-отправка: ClientRPC шлёт только изменённые предметы
- [ ] При подключении клиента — актуальные цены рынка
- [ ] Коммит и пуш
