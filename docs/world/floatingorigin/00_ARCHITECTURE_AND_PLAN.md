...
- Следующий интеграционный фокус: подтверждённый trusted account issuer/его реальные вызовы и подготовка конкретного pilot scope в основном build — catalog/markers/frames/physics, player prefab/layout/profile, отсутствие legacy position services и конкурирующих writers, explicit store/configuration. Код source и session/persistence orchestration теперь есть, но actual configuration/native acceptance отсутствуют. Остаются AOI/streaming, ship/ParentLocal composition, RPC/DTO, individual disconnect final-save/respawn, client rebase, multi-region physics/Host, NavMesh/AI, visual/global-height consumers и user acceptance. Полные T-FO04–09 и semantic T-FO03 не закрыты; world shift выключен, jitter fixed не заявляется.
