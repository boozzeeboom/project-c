...
> **Что нового в v0.0.28 (20 июня 2026):** **Docking Stations — MVP!** Полный цикл стыковки: игрок подлетает в OuterCommZone (1000м вокруг станции) → жмёт **T** → CommPanel справа → **[Запросить посадку]** → диспетчер назначает pad по совместимости класса корабля → **[Хорошо]** → 5 минут окно на посадку → летит к pad'у → касание trigger-box → **двигатель блокируется**. В **Docked**: W/A/S/D не работают; отстыковка через T → **[Отстыковка]** → двигатели разблокированы. Серверный hub (`DockingServer`) + SOT (`DockingWorld` с `ScanExistingOccupants()` при старте — корабли на падах регистрируются как occupants) + физическая проверка занятости падов (Physics.OverlapBox). UI Toolkit окно `CommPanelWindow` (справа, не модальное) + канонический `docs/UI/UI_TOOLKIT_GUIDE.md` для всех будущих окон. HUD column **DISPATCH** в `ShipHudController` показывает состояние связи (красная/зелёная точка, stationId, "T — связаться"). Двусторонняя связь с диспетчером (`RequestConfirmAssignmentRpc`). Initial-scan + двигатель-блокировка через `ShipController._netIsDocked: NetworkVariable<bool>` + `_rb.isKinematic = true`. ⚠️ Визуальные маркеры падов (`DockPadVisualMarker`) — Quad создаётся, но цвет не меняется: требует переработки (тикет `T-DOCK-14`). Полный аудит + 5 фаз рефакторинга в `docs/Docking_stations/AUDIT_AND_REFACTOR.md`. 0 compile errors.

> **Предыдущее (v0.0.27):** **Ship Key v2 (R2-SHIP-KEY-003, MVP завершён v18–v20).** Уникальные экземпляры ключей к кораблям — каждый из 3 кораблей (Light/Medium/Heavy) получил свой уникальный `KeyRodInstance` через scene-placed `KeyRodInstanceBinding`. Серверный реестр `KeyRodInstanceWorld` гарантирует 1:1 привязку (instanceId ↔ shipId ↔ ownerPlayerId). Подбор/дроп с передачей instanceId, persistence через `JsonKeyRodInstanceRepository` (`KeyRodInstances.json`). Drop↔pickup реактивирует Lost instance (не создаёт дубль). UI вкладка "КОРАБЛЬ" в CharacterWindow показывает только корабли игрока (`MyShipsTab`); TAB-колесо: сектор 1 = "ВЛАДЕНИЕ" (Equipment + Key). Серия фиксов: T-KEY-06/07/08/09, Phase C/D (reflection removal + serialization fix), Phase E (retry-loop). 0 compile errors. См. `docs/Ships/Key-subsystem/99_CHANGELOG.md` (v1–v20) + `28_KEY_ARCHITECTURE_REVIEW.md` (глубокий анализ).

> **Предыдущее (v0.0.26):** **Composite Ship Architecture — Phase 0+1.** Корабль больше не монолитный блок — теперь составная конструкция (летающая баржа). `ShipRootReference` (маркер части корабля), `ShipComponentLocator` (поиск ShipController), `PilotSeatController` (место пилота как отдельный child), `DoorController` (slide E-key). Игрок НЕ пропадает при посадке — стоит в кресле, видимый. Парентинг к корню корабля — физика плавная, без дерганий. Камера следит за ShipRoot, а не за игроком. Полный анализ: `docs/Ships/analysis-composite-ship.md`.
...
| [`docs/Ships/Key-subsystem/`](docs/Ships/Key-subsystem/) | **Ship Key v2** (R2-SHIP-KEY-003, v18-v20 MVP) — уникальные экземпляры ключей к кораблям. 20 файлов: changelog (v1-v20), планы (T-KEY-08/09), architecture review, refactor plan, setup guide. |
