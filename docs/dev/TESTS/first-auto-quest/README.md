...
> Внутренний гайдлайн для реализации и исправления первого onboarding-квеста в `ProjectC_client`.
>
> Назначение документа: при следующей правке не восстанавливать архитектуру по памяти, а пройти последовательность ниже, проверить контрольные точки и дополнять этот файл найденными ошибками.
...
- Полный проход квеста в Play Mode.
- Реальное переключение всех диалоговых узлов.
- Реальное прохождение статичных `ReachLocation` в игре.
- Реальное срабатывание `TalkToNpc` для Mira после возврата с маршрута.
- Появление `Key_light_ship` в инвентаре после turn-in.

Play Mode-проверку выполняет пользователь вручную. До такой проверки нельзя считать квест полностью runtime-проверенным.
...
6. Для посещения точек маршрута использовать objectives типа `ReachLocation`.
7. Не сохранять сцену в новый корневой путь вроде `Assets/WorldScene_0_0.unity`. Канонический путь сцены:
   `Assets/_Project/Scenes/World/WorldScene_0_0.unity`
8. После изменений сцены проверить, что активная сцена снова имеет канонический путь и в проекте не остался случайный дубль.
9. Не считать прохождение задачи завершённым только по успешной компиляции: runtime-flow всё равно должен быть проверен в Play Mode пользователем.
...
Пока пользователь не подтвердил этот сценарий, статус должен оставаться: `compile-validated, runtime-not-tested`.
...
6. `QuestWorld.HasNpcTalkedTo` получает тот же `npcId`, который указан в objective.
...
- [ ] Play Mode проверен пользователем или явно помечен как не проверенный.
- [ ] Новые найденные ошибки добавлены в этот файл.
- [ ] Commit hash записан в `docs/iterations.md`.
...
- Runtime-check: не выполнялся, ожидается ручной Play Mode-тест.
