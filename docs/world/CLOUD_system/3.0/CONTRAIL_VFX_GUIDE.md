...
### 3.6 Alpha over Life — Устранение отрыва трейла 🔑

**Проблема:** при остановке корабля эмиссия прекращается (Stop), и последние частицы создают жёсткий непрозрачный край в месте отрыва.

**Решение:** добавить fade-in в начале жизни и fade-out в конце.

**Где:** `Update Particle` → добавить блок `Alpha over Life`

**Кривая:**
```
0%:    alpha = 0    ← fade-in (частица появляется плавно)
15%:   alpha = 1    ← полная непрозрачность
70%:   alpha = 1    ← держится
100%:  alpha = 0    ← fade-out (растворяется)
```

**Альтернатива через Set Color:** в `Initialize Particle` → `Set Color` → Gradient:
```
0%:    RGBA(1, 1, 0.95, 0)       ← прозрачный старт
10%:   RGBA(1, 1, 0.95, 0.8)     ← быстрое появление
50%:   RGBA(0.9, 0.9, 0.95, 0.4)
100%:  RGBA(0.8, 0.85, 0.9, 0)   ← растворение
```

**Кодовая страховка:** `ShipContrailVfx.StopDelay = 0.4s` — задержка перед Stop(), даёт последним частицам время на fade-in.
