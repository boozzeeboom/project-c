...
| 2026-08-14 | Bugfix | **Respawn race (active-only Animator) ✅:** `SkillAnimationPlayer.Awake` ищет Animator только по активным объектам (`GetComponentsInChildren<Animator>(false)`), т.к. при respawn компонент добавляется после swap, а старый body жив до конца кадра (deferred `Destroy`). | T-CUS-03 |
| 2026-08-14 | Bugfix | **Скиллы не игрались после смены пола ✅:** `SkillInputService` кэшировал `_animator` один раз (reflection из `NetworkPlayer._animator`); после whole-model swap старая ссылка указывала на destroyed Animator, и legacy `SetTrigger("Attack")` уходил в никуда. Добавлена подписка на `BodySwapped` → `_animator = newAnimator`. | T-CUS-03 |
