...
Create `.QWENCODE/settings.local.json` for personal overrides that should NOT
...
QWENCODE supports different permission modes. Recommended for game dev:

### During Development (Default)
Use **normal mode** — QWENCODE asks before running most commands. This is safest
for production code.
...
### During Code Review
Use **read-only** permissions — QWENCODE can read and search but not modify files.
