...
> ⚠️ Обновление 2026-09-21: §«Текущий регламент» ниже (контент без маркеров)
> **отменён для объектов с `NetworkObject`** — см. `09S_NEW_CONTENT_PIPELINE.md`
> (кейс парома: Host падает без регистрации). Для простого контента без NO
> регламент действует как раньше.
