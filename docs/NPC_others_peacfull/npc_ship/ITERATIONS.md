...
Berthing 7%, WallFollow **4%**, Avoids 3%, Lifting 1%. Планы: `graph` **20**
(1/2/3), peak 14, direct 3. `graph-fail` всего 4. `touchdown` ×2 (0026, 0012).
`lift-chimney-blocked` сработал (0009 → divert). `berth-abort` 7 (клифовые
станции, цепочки терминируются). dead-divert ×2 на самом конце (000A, 0031 —
краевые случаи, терминированы). Наблюдение пользователя «стали облетать гору»
подтверждено цифрами.

**Вердикт по магистралям: НЕТ (сейчас).** Обоснование: проблема, ради которой
проектировались магистрали (слепые детуры), решается меш-моделью на глазах:
graph-планы 0 → 20 за три сессии, wall-time 4%. Оставшаяся боль (berth-abort
у клифов, редкие dead) — терминальная фаза, магистрали её не касаются
(они про en-route). Цена (200 LOC + боксы + валидатор + поддержка при правках
станций + матрица тестов) против косметического выигрыша — не сходится.
Критерий возврата: после оседания MESH01 (2–3 длинных прогона) детуры всё ещё
безобразны ИЛИ магистрали нужны как геймплей-фича (торговые пути), а не фикс.

**Наблюдения в бэклог (без действий):** `too-many-discs=9` ×1 (лимит 8 дисков
графа) — поднять 8→10, только если повторится. Мораторий на тактику — держать.

---

## Итерация от 2026-09-25 — T-NS-LOG02g: разбор NpcShipNavLog_20260925_172716 (5.1 мин)

**Факты (349 TRANS, старт пачкой из одного города):** полный цикл взлёта у всех
20 (`dwell→lift→yaw→cruise`); `probe` 39 / `LOS` 38 (97%!); **0 дивертов/dead/
abort** — навигация здорова. Соцшторм 23→11→4→4→2→2/мин — стартовый транзиент
пачки (depart-mutex разгребает), к 4-й минуте ~2/мин (< цели 5/мин): чинить нечего.
`graph-fail` 22, `nav-wp` 18, mesh-диски в деле (discs до 6). Лорейн здорова
(avg 13.4, 13 обходов) — аномалия была геометрией leg'а, не кораблём (понижен приоритет).

**Единственная дыра — chimney без watchdog (0009):** 5 мин в Lifting, y const
(прыжки трейла — F8/F9 пользователя), пад под козырьком. `TickLift` не имел
предохранителя вообще (DEPART3 предполагал открытое небо).

**Фикс — lift-chimney-blocked:** набора < 10 м дольше 40 с → `DivertToNextStation`
(низом разберутся probe/стена/круиз-цепочки; расписание round-trip вернёт позже).
Якорь: вход в Lifting; сброс при F8 (якорь не мировой — иначе сдвиг врёт замер).
Норма chimney 60 м за 6–12 с, окно 40 с — ложных срабатываний не будет.

**Проверка:** `refresh_unity` → `error CS` 0. Play Mode — за пользователем (NOT RUN).
Следующий замер: `lift-chimney-blocked` у потолочных падов, 0009-тип ≤ ~50 с.

---

## Итерация от 2026-09-25 — T-NS-LOG02f: разбор NpcShipNavLog_20260925_150147 (2.4 мин)

**Факты (113 TRANS):** самая спокойная сессия: 0 дивертов/dead/abort;
`probe` 20 / `LOS` 16 (80%); `nav-wp` 10; `graph-fail` 16 (диски 4–6 —
бейк мешей работает: знание есть); социальный слой активен
(yield/avoids — все разрулены). Единственный гринд — 0032: 90 с вдоль стены
на 2.8 м/с (dist +220 м) мимо всех вотчдогов.

**Корень (one-shot дыра):** wall-noprogress меряет от фиксированного входа
(100 м за первые 35 с). 0032 закрыл ~91 м к 35-й секунде — промах на метр,
иммунитет навсегда. Контурный гринд в обходе не покрыт ничем
(goal-watchdog из LOG02c работает только в Cruising).

**Фикс — rolling-замер в WallFollow:** сближение с NavGoal < 1 м/с дольше
45 с (не в retreat) → сразу `StuckDivert wall-goal` (цепочка; replan в стене
бессмыслен). Часы: вход в обход; выходы — через SetMode. Старый one-shot
оставлен (ловит быстрый стендап раньше). Лорейн: префабы идентичны, пилотов
не было, дубликатов нет — нужен рантайм-осмотр (debugMode + консоль), остаётся open.

**Проверка:** `refresh_unity` → `error CS` 0. Play Mode — за пользователем (NOT RUN).
Следующий замер: `wall-goal` срабатывания, 0032-типа эпизоды ≤ ~50 с.

---

## Итерация от 2026-09-25 — T-NS-MESH01 + B1: меши в дисках, память в tangent

**A (MESH01, реализовано):** `PeakRegistry` бейкает меши-скалы в диски:
`MeshCollider` (non-convex ок — выпуклость не нужна), не-триггеры, без кораблей,
XZ-диагональ ≥ 200 м. Диск = центр XZ + радиус (½ диагонали × 1.2, кап 2000)
+ верх bounds. FO-safe: храним Transform + локальный центр/радиус/высоту,
мир — резолвом вживую; инвалидация — один раз за сдвиг из
`NpcShipTrafficManager.ApplyRebaseTranslation` (не per-ship: иначе 20
пересканов). `FindBlockingDisc`/`CollectBlockingDiscs`/`IsInsideDisc`
учитывают меши тем же предикатом (верх вместо вершины). Лог билда:
`N peaks + M meshes`. Станция внутри городской геометрии = mountain-home
(заход напрямую — правильно для дока в городе).

**B1 (память в tangent, реализовано):** `TangentBypass` читает hotspot-штрафы
сторон (×0.05 к длине; без hotspot — бит-в-бит). Leg туда гриндовал — leg назад
пробует другую сторону кольца. Опечатка-фикс по ходу: порог ничьи возвращён
к 0.95 (было задето 1.05).

**B2 (персистентность трасс — ОТЛОЖЕНО, честно):** требует интеграции с форматом
сейвов (T-PERSIST: ключи, кумулятив, схема) — отдельным тикетом после замера A.
Внутрисессионная память (hotspot 10 мин) покрывает пинг-понг частично.

**Дизайн 17 (магистрали):** `17_AIRWAYS_DESIGN.md` — боксы = топология
(авто-связи по перекрытию), ведение = центр→центр существующей цепочкой,
короткие плечи не пользуются (дистанция + стоимость), повороты = Дейкстра.
Реализация (AIR01) — следующим тикетом, по отмашке.

**Проверка:** `refresh_unity` → `error CS` 0 (после реконнекта бриджа).
Play Mode — за пользователем (NOT RUN). Следующий замер: `M meshes > 0`
в консоли сервера, падение `graph-fail no-path`, `how=graphN` рост.
