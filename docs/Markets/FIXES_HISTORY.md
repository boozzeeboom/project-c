...
**Серверная сторона:**
- `Assets/_Project/Trade/Scripts/Network/ContractServer.cs` (19 КБ) — `NetworkBehaviour`, RPC hub, ставится в `BootstrapScene` рядом с `MarketServer`. Методы: `RequestListRpc`, `RequestAcceptRpc`, `RequestCompleteRpc`, `RequestFailRpc`. Server→owner delivery выполняется через `NetworkPlayer.ReceiveContractSnapshotTargetRpc` и `ReceiveContractResultTargetRpc`. `FixedUpdate` тикает таймеры активных контрактов + авто-fail по `TimerExpired`. `MaxActiveContractsPerPlayer=3` + rate limit `30 ops/min` (FIX F2).
