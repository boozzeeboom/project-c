...
namespace ProjectC.EditorTools.FloatingOrigin
{
    ...
    public static class ValidateGlobalSceneExecution
    {
        ...
        public static Report Run()
        {
            ...
            Check("I protocol rejects H and legacy is not reserved", () => { Require(GlobalMotionNetworkContract.ProtocolVersion == 0xF005 && !GlobalMotionNetworkContract.IsReservedProtocolVersion(0)); var d = new byte[32]; d[0] = 1; var current = GlobalMotionNetworkContract.CreateHello(d, d); var previous = (byte[])current.Clone(); previous[6] = 4; previous[7] = 0xf0; Require(!GlobalMotionNetworkContract.ValidateHello(previous, current, out _)); });
