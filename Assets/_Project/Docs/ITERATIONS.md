...
**Результат:** 3 параллельных инвентаря (01–05, 06-серия, 07/08/09/tools). В архив `archive_floatingoriginintegration/`: 59 md процессных (capture-серии, микрошаги readiness, superseded), 117 evidence-json, 3 csv, 7 разовых Editor-тулз (dormant). Свёртки с сохранением смысла: 07I/07J→08C, 07D/07G→09O, 09B_PLAN→09B_IMPL, 09E→09D. Новый `README.md` — точка входа (как работает, что читать, что не закрыто, что в архиве). Главный каталог: 123 md, все KEEP-вердикты.

**Проверка:** Без кода (только доки, git mv сохраняет историю). Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/README.md`, `08C/09O/09B_IMPL/09D` (приложения), архив.

---

## Итерация от 2026-09-14 (T-FO09O-verify — ф8_36 штатно + аудит шумных логов)

**Задача:** ф8_36 точечно; выключить шумные FO-логи; коммит если штатно.

**Результат:** Штатно: 2× auto `Completed` (frame 2→3, все маркеры ×2 — Corridors/Wind/RespawnPoints ровно по одному на транзакцию, 09G-fix держит), `ActorRebound` 2× ok=True, 1 одиночный легитимный респаун (падение −2724 → точка, без циклов), grounded преимущественно, 0 errors. Аудит шумности: все добавленные мной логи — низкочастотные (на сдвиг/сессию/фейл); per-frame идёт только T-FO06Y probe (выключает пользователь); Wind-лог за флагом `_logWindChanges` (pre-existing). Выключать кодом нечего — без изменений кода.

**Проверка:** Точечный разбор ф8_36. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** Корень — rollback возвращал контент, но не книги: кумулятив сейва рос фантомом на каждый F9, свежий старт вычитал фантом → смещение спавна. Дизайн PERSIST01 требовал `-= T`, реализации не было. Фикс: при `restored` — `_frame = Plan.Before` + `CumulativeRebaseOffset -= T` (generation монотонна, lineage цел). Сейв атомарен, Bootstrap чист, стриминга нет — проверено и отброшено.

**Проверка:** `refresh_unity` — PASS; `read_console` — 0 errors, 0 CS. Play Mode retest — NOT RUN (user: F8→F9→выход→заход → спавн в городе).

**Файлы:** `.../Network/GlobalMotionControlledRebaseSlice.cs`, `docs/world/floatingorigin/09K_ROLLBACK_BOOKS_FIX.md`, `Assets/_Project/Docs/ITERATIONS.md`.

---

## Итерация от 2026-09-14 (T-FO09J-verify — спавн на старой позиции, PASS)

**Задача:** ф8_33: проверка персистенции позиции после 09J.

**Результат:** PASS. `PilotSpawnRestored: client`, старт `origin=(39936,2560,40192); local=(-14.86,-57.84,-69.84)` — игрок на старой позиции (не дефолт), сразу автосдвиг (frame=2, резолв до троттлинга работает), дальше ходьба до (162,-57,122), `grounded=True` 2404, движение 699, 0 errors, 0 телепортов. Замечание (не блокер): текущий сейв Y=-1210 — выход был в падении; следующий старт покажет падение→респавн (штатно) или точку (EnsureDeathBelow). Без кода.

**Проверка:** Точечный разбор ф8_33. Компиляция не затрагивалась.
...
**Результат:** ф8_28 ничего не тестирует: сейв на входе отравлен багом ф8_26 (−2M → `outside pilot frame, default spawn`), 09H включиться не мог, restore кораблей в логе не завершён. Текущий сейв здоров (`inShip=True`, корабль-копия, 63.4/39.4/59.9) — настоящий тест 09H впереди. Риски: conflict-имя префаба (переименовать в редакторе), restore только двигает scene-корабли (спавна из сейва нет), `SaveNow` скипается при `wasGlobal`, флот/игрок в разных порядках — ждать лог. По плану: 08C отложен, 06CM–CW dormant/BLOCKED, docking/recall — OPEN. Без кода.

**Проверка:** Разбор ф8_28 + прямой анализ сейва + статика. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/09I_SHIP_PERSISTENCE_MAP.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** 09C работает штатно: ship-цепочка не сработала (нет кораблей рядом), fallback честно телепортирует на `fallbackPosition=(39835,2533,40003)` → падение (`grounded=False`, ускорение плавное). Пилотный старт той же сессии спавнится в `(39835,2534,40003)` — `grounded=False` с первого кадра: точка висит в воздухе изначально, не после сдвига. Геометрия по YAML: ближайший хангар ≈ (40315,2502,39869), точка от него на ~480м по X. Контент: поставить fallback на верх доказанной геометрии (палуба хангара ~+1м или Deck-платформа). Без кода.

**Проверка:** Разбор ф8_25 + YAML-расчёт геометрии. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/09E_RESCUE_FALLBACK_POINT_GEOMETRY.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.

---

## Итерация от 2026-09-13 (T-FO09D — точка спасения висит над пустотой, без кода)

**Задача:** Разобрать ф9_24 («спасение не возвращает на спавн»).

**Результат:** Спасение сработало (2× `TeleportRpc.end(target=[39835,2533,40003])`, 09C-поведение подтверждено) — но точка не стоит на коллайдере: после телепорта `grounded=False`, тонет ~56 м/с, оба раза одна точка → цикл. Единственный [Error] — Unity Editor TextSelection GUI-шум. FO здорова (deathY едет, кэши в норме). Виноват контент: позицию точки в редакторе ставить на верх BoxCollider (hangar walls / Deck-платформы). Сцена кодом не трогается. Без кода.

**Проверка:** Точечный разбор ф9_24 + YAML. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/09D_RESCUE_POINT_VOID.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** НЕ баг FO. Факты: `grounded=False` все 2927 записей; оба сдвига штатны (`auto_threshold` frame=2 — автотриггер сработал как designed — + `user_controlled` frame=3, 0 errors); `Respawn_Default` в сцене = `(39992, 0, 40000)` — столб без земли (GroundPlane_0_0 удалён ещё в 06K, задокументированный риск). Игрок спавнится в y=1 над пустотой → падает → респаун в ту же пустоту (цель `(39992,2502,40000)` — там тоже нет земли) → падает дальше. F8/авто двигают мир штатно, но землю не создают. Доказательство: PERSIST-verify стоял в `(40121,2502,40043)` без падений — земля рядом есть, точка спавна мимо неё.
Предложение (не код, контент): перенести `Respawn_Default` в редакторе на доказанную землю (~40121,2502,40043) или восстановить граунд. Сцену кодом не трогаю. Без кода.

**Проверка:** Точечный разбор лога + YAML сцены. Компиляция не затрагивалась.
...
**Результат:** Остаток кодового — один крупный gate (авто-порог) + один микрофикс (trails в clear-путь, батчем). Всё остальное: PASS, тесты пользователя или deferred-мультиплеер. Микро-аудиты: combat DTO транзиентны, altitude live — правок не надо. План 09B: таймер 5с + кулдаун 35с + тот же путь транзакции с `reason=auto_threshold`; гистерезис бесплатен из `TryCreate`; F8-эквивалентность по безопасности. Без кода.

**Проверка:** Чтение плана §2–§5 целиком + точечные grep (трайлы, DTO, altitude, пороги). Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/09B_AUTO_REBASE_PLAN.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** PASS с оговоркой. Лог чист: 1× `Completed`, 0 errors, `ActorRebound(ok=True)`, оба `inShip` состояния, 2 одиночных респауна (легитимные смерти: падение до сдвига + бой после; циклов нет — DB держит). 34× `Rejected` — все `no_rebase_plan` у origin (честные отказы, не баг). Оговорка: это НЕ доказывает повторный дальний F8 в пределах 30с (кулдаун 06DF) — rejects здесь по близости, а не по кулдауну; far-repeat остаётся OPEN. Прыжок/бой после сдвига — визуально со слов пользователя, в логе отдельной метрики нет. Без кода.
...
## Итерация от 2026-09-13 (T-FO08F-verify — дальние координаты, джиттер лечится F8)

**Задача:** Разобрать f8_20 (полёт на высокой скорости, отлёт 5000+, F8 в глобально разных координатах).

**Результат:** PASS, ключевое доказательство ценности FO: 4× `Completed` в сильно разных глобальных координатах (origins `[39936,0,39936]→[32000,0,36608]→[7424,0,1536]→[-38656,0,41216]`, frame 2→5), 0 errors, 0 `respawn.TeleportRpc`, 4× `ActorRebound(ok=True,placed=True)`. Локальные позиции игрока остаются сотнями метров (локальный фрейм держит), горы/мир на местах. 1× `Rejected(no_rebase_plan:localFocus=[114.86,1.00,-50.81])` — честный отказ у origin, не баг.
Ответ на вопрос про персистенцию: каждый заход со спавна — это отсутствие save/load персонажа, отдельная подсистема вне FO (матрица §5, OPEN). FO её не ломает и не чинит; фиксить отдельным треком, не в серии FO. Без кода.
...
## Итерация от 2026-09-13 (T-FO08E — решение: мультиплеер отложен, фокус на хост-одиночку)

**Задача:** Разобрать ф8_19 («второй клиент видит пустую бутстрап-сцену»).

**Результат:** Второго игрока в логе нет вообще: единственный `HandleClientConnected: clientId=0 (IsServer+IsClient)`, `ConnectedClients.Count=1`, ни одного `REMOTE_`/`ClientShiftApplied`. Пустая сцена у второго клиента — проблема подключения/сетапа вне FO (игрок не присоединился), чинить в FO нечего. Решение пользователя: всю отладку >1 игрока отложить, концентрируемся на полностью рабочем хост-клиенте с FO. Код DH/08D остаётся dormant (NOT RUN), приёмка по нему не заявляется.
Попутно из ф8_19: F8 здоров (Completed, teleported=58, 0 errors), `ActorRebound(ok=True,placed=True)` — пеший rebind успешен. Без кода.
...
**Результат:** `09A_ACCEPTANCE_MATRIX.md`: 8 разделов, статусы PASS/PARTIAL/OPEN/BLOCKED с доказательством (лог/тикет) на каждый подпункт. PASS: F8/F9-цикл, ходьба, палуба, полёт (визуально), палубы, respawn, rollback, пикапы, камера, discontinuity. Ближайшие достижимые: кулдаун 30с, прыжок/бой после, шторм, посадка/выход через сдвиг, замеры. Блокеры: второй клиент, late join, save/load, ship-adapter, приёмка целиком. Живой документ.

**Проверка:** Без кода — сводка по существующим отчётам и логам. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/09A_ACCEPTANCE_MATRIX.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** Действий не требуется. Посаженный игрок едет с корнем-участником (отдельно не регистрируется — покрыт `IsContainedByRegisteredRoot`); сдвиги deathY/платформы ему положены и безвредны; rebind читает мировую позицию; выход — live `GetExitPosition()`; ссылки — object refs. NPC parented-ветка кэш лишь перечитывает; cargo/visuals — children.

**Проверка:** Без кода — статический аудит. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/07G_BOARDING_PARENTING_AUDIT.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** PASS. Первый F8 — полный успех (`Completed`, frame=2). Второй — корректно `Rejected(no_rebase_plan:localFocus=[50.43, -57.84, 63.82])`: игрок у origin, порог 256 не превышен, транзакция не стартовала (ни Apply, ни маркеров). Игрок стабилен, телепортов после 0, ошибок 0. Честная оговорка: путь кулдауна палуб DF этим тестом напрямую не задет (rejection до движения — дрейфа нет); для него нужен отход >256м и второй F8 в пределах 30с. Без изменений кода.

**Файлы:** `Assets/_Project/Docs/ITERATIONS.md`.

---

## Итерация от 2026-09-13 (T-FO06DI — блокер global-фрейма, без кода)

**Задача:** Проверить инвариант global-потока пилота через F8: публикуемая global-позиция должна сохраняться (план §2.1).

**Результат:** БЛОКЕР подтверждён статически: `TryCaptureWorld → ToGlobal(local)`, а `Frame.Origin` immutable и не едет со slice → публикуемая global прыгает ровно на translation (поток жив: revision/sequence растут в ф8_10). Потребитель (checkpoint source T-FO06C) сохранил бы прыжок. Латентно: в живой игре потребителя global-потока нет (T-FO06B не подключён), single-host не страдает. Точечный фикс отклонён: требует ломки immutable-ядра + 600 pure-проверок; хак через `ReactivateFromCurrentPose` легализовал бы прыжок. Нужен gate уровня T-FO07 (замена фрейма + перепривязка акторов). Критерии приёмки зафиксированы в отчёте.

**Проверка:** Без кода — аудит + revision/sequence из ф8_10. Компиляция не затрагивалась.

**Файлы:** `docs/world/floatingorigin/06DI_GLOBAL_FRAME_ORIGIN_BLOCKER.md`, `Assets/_Project/Docs/ITERATIONS.md`.

---

## Итерация от 2026-09-13 (T-FO06DH-verify — хост-тест по ф8_10)

**Задача:** Проверить DH-код на хосте с одним игроком (мультиплеер пока NOT RUN).

**Результат:** PASS. Полная цепочка, `BroadcastShifted(sent=True,frame=2)` при нуле клиентов, гард хост не заблокировал, телепортов 0, ошибок 0, визуально ок. Без изменений кода.

**Файлы:** `docs/world/floatingorigin/06DH_HOST_AND_SECOND_CLIENT_PREP.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** PASS полностью. Было `[40210.81, 2504.69, 40075.14]` → стало `[18.81, -55.32, -116.86]` стабильно; `onPlatform=True` все 583 записи после; `PlatformCarry` дельты сантиметровые, флинга нет; телепортов 0, ошибок 0. Без изменений кода.

**Файлы:** `docs/world/floatingorigin/06DG_PLATFORM_CACHE_SHIFT.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** PASS полностью. `DecksNotified(decks=20)`, все 20 палуб `Registered at` в сдвинутых координатах, игрок стабильно `(56.004, -57.835, 64.000)`, телепортов 0, ошибок 0. Без изменений кода.

**Файлы:** `docs/world/floatingorigin/06DF_DECK_REBASE_NOTIFICATION.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** PASS полностью. `RollbackCompleted`, `NetworkPublished(teleported=58,skipped=0,errors=0)`, `RespawnShifted` отсутствует, `deathY=0.000` стабильно, телепортов после rollback — 0, игрок стоит на `(39992.01, 2502.17, 40000.00)`, ошибок — 0. «Ничего визуально не произошло» = корректный результат F9 (apply + restore, net zero). Изменений кода нет, только верификация.

**Файлы:** `docs/world/floatingorigin/06DC_RESPAWN_SHIFT_FLAG.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Файлы:** `docs/world/floatingorigin/06CX_RUNTIME_CAPTURE_REVIEW_02.md/.json`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** Созданы `GlobalMotionShipDeckPassengerLifecycleFact`, immutable `GlobalMotionShipDeckPassengerLifecycleCoordinatorReceipt` и sealed `GlobalMotionShipDeckPassengerLifecycleCoordinator`, реализующий `IGlobalMotionShipDeckPassengerGenerationSource`. Coordinator принимает только explicit server-authorized/protocol-owned `ShipRegistered`, `PassengerAttached`, `PassengerDetached` и terminal `ShipInvalidated`; сам выдаёт monotonic ship lifetime и passenger attachment generations, strict ledger ordinals, сохраняет invalidation terminal state и fail-closed выдаёт active passenger generation view. Stale, duplicate, mismatched, out-of-order и post-invalidation facts отклоняются. Pure Edit Mode validator: `11/11` PASS; `check_compile_errors=No compile errors`; script validation `0 warnings / 0 errors`.

**Граница:** Coordinator остаётся dormant и не связан с `NpcShipController`, `ShipCrewSpawner`, `NpcBrain`, `ShipDeckNav`, `GlobalMotionShipDeckCombinedTransactionHost`, provider, adapter set, `BootstrapScene` или runtime driver. Synthetic generation из `IsSpawned`, `NetworkObjectId`, object references, `ShipDeckNav.RegistrationGeneration` и host-local binding generation не используется. Сцены/префабы не изменялись, BootstrapScene unchanged, Play Mode не запускался; `runtimeRebaseReadiness=NOT_READY`.

**Файлы:** `Assets/_Project/Scripts/World/FloatingOrigin/Network/GlobalMotionShipDeckPassengerLifecycleCoordinator.cs`, `Assets/_Project/Editor/FloatingOrigin/ValidateGlobalMotionShipDeckPassengerLifecycleCoordinator.cs`, `docs/world/floatingorigin/06CN_SHIP_DECK_LIFECYCLE_COORDINATOR_IMPLEMENTATION.md/.json`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`, `Assets/_Project/Docs/ITERATIONS.md`.
...
**Результат:** Добавлены `GlobalMotionShipDeckPassengerLifecycleBindingHandoffContract`, pure validator и explicit host API `TryConfigureReviewedPassengerLifecycleBinding` / `TryValidateReviewedPassengerLifecycleBinding`. Handoff проверяет passenger count, binding generation, ship lifetime identity и ownership. Validator: `8/8` PASS; `check_compile_errors=No compile errors`; script validation `0 warnings / 0 errors`.

**Граница:** Handoff принимает только explicit reviewed receipts и остаётся dormant. `NpcBrain` и `ShipDeckNav` не изменялись, runtime producer/source binding/adapter registration/BootstrapScene/Play Mode не выполнялись; `runtimeRebaseReadiness=NOT_READY`.

**Файлы:** `Assets/_Project/Scripts/World/FloatingOrigin/Network/GlobalMotionShipDeckPassengerLifecycleBindingHandoffContract.cs`, `Assets/_Project/Editor/FloatingOrigin/ValidateGlobalMotionShipDeckPassengerLifecycleBindingHandoffContract.cs`, `Assets/_Project/Scripts/World/FloatingOrigin/Network/GlobalMotionShipDeckCombinedTransactionHost.cs`, `docs/world/floatingorigin/06CI_SHIP_DECK_PASSENGER_LIFECYCLE_BINDING_HANDOFF.md/.json`, `Assets/_Project/Docs/ITERATIONS.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`.
...
**Результат:** Создан `GlobalMotionNetworkBaselineReversibleTransactionContract` и pure Edit Mode validator. Capability требует Restorable evidence, server authority, protocol ownership, explicit capture/apply/validate/restore boundaries, ownership/lifetime/baseline-generation restore и transaction identity. Receipt проверяет ordered phase lineage; `None`/`Faulted` и пропуск ранних границ отклоняются. Фактическая проверка: `16/16` pure checks PASS, `check_compile_errors=No compile errors`.

**Граница:** Контракт не читает и не меняет NGO state, `GlobalMotionReplicator`, adapter set или Unity scene. Concrete NetworkBaseline producer/adapter, ShipDeckNav adapter, provider binding, runtime installation, rollback evidence и Play Mode не выполнялись. `GlobalMotionNativeAdapterSet=EMPTY / UNSEALED`, `runtimeRebaseReadiness=NOT_READY`.

**Файлы:** `Assets/_Project/Scripts/World/FloatingOrigin/Network/GlobalMotionNetworkBaselineReversibleTransactionContract.cs`, `Assets/_Project/Editor/FloatingOrigin/ValidateGlobalMotionNetworkBaselineReversibleTransactionContract.cs`, `docs/world/floatingorigin/06BN_NETWORK_BASELINE_REVERSIBLE_CAPABILITY.md/.json`, `Assets/_Project/Docs/ITERATIONS.md`, `docs/world/floatingorigin/00_ARCHITECTURE_AND_PLAN.md`.
...
**Результат:** `GlobalMotionRebaseRuntimeDriver` получил `TryAuthorizeInstallationIntent`, `IsInstallationIntentAuthorized` и `InstallationIntent`. User-controlled request теперь требует readiness authorization и exact match installation intent по manifest/session identity. `TryReset` очищает installation authorization.

**Граница:** Valid bundle, concrete adapters, manifest publication, participant discovery, Unity mutation, Apply/Rebuild/Validate/Publish, rollback, BootstrapScene и Play Mode не изменялись. `validate_script` — PASS; compile — PASS; `git diff --check` — PASS; runtime rebase readiness остаётся `NOT_READY`.

**Файлы:** `GlobalMotionRebaseRuntimeDriver.cs`, `docs/world/floatingorigin/06AL_DRIVER_INSTALLATION_AUTHORIZATION.md/.json`, `00_ARCHITECTURE_AND_PLAN.md`.
