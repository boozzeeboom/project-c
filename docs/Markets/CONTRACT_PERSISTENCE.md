...
## Solution
Расширен `IPlayerDataRepository` двумя методами: `SaveContracts(ContractSaveData)` / `TryLoadContracts(out ContractSaveData)`. Добавлен `ContractSaveData` — `[Serializable]` DTO, агрегирующий всё состояние `ContractWorld`. Реализовано в обоих репозиториях (`ServerFileRepository` → JSON-файл, `PlayerPrefsRepository` → PlayerPrefs).

`ContractWorld` теперь:
- При `Initialize`: сначала `LoadAll()` из репозитория; если данных нет — генерирует новые и сохраняет
