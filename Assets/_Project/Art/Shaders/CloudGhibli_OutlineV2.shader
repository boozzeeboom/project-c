// CloudGhibli_OutlineV2.shader — Ghibli cloud with single-pass screen-space outline
// Features:
// - Screen-space edge detection outline (no second pass needed)
// - Gradient lighting
// - Rim glow
// - Vertex displacement
//
// Uses depth-based edge detection in a single pass for reliable outline

Shader "ProjectC/CloudGhibli_OutlineV2"
{
    Properties
    {
        [Header(Base Color)]
        ...
        _OutlineWidth ("Outline Width", Range(0.5, 3.0)) = 1.5
        _OutlineThreshold ("Outline Threshold", Range(0.01, 0.2)) = 0.05
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half DetectOutlineEdge(Varyings input, float2 screenUV)
            {
                float2 texelSize = _OutlineWidth / float2(1920, 1080);
                
                float depth = input.depth;
                
                float4 d = float4(
                    SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, screenUV + float2(0, texelSize.y)).r,
                    SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, screenUV + float2(texelSize.x, 0)).r,
                    SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, screenUV - float2(0, texelSize.y)).r,
                    SAMPLE_TEXTURE2D(_CameraDepthTexture, sampler_CameraDepthTexture, screenUV - float2(texelSize.x, 0)).r
                );
                
                float depthDiff = abs(depth - d.r) + abs(depth - d.g) + abs(depth - d.b) + abs(depth - d.a);
                
                float normalEdge = 0;
                float3 n = input.normalWS;
                float3 nR = normalize(float3(n.x + 0.1, n.y, n.z));
                float3 nU = normalize(float3(n.x, n.y + 0.1, n.z));
                normalEdge +=1.0 - saturate(dot(n, nR));
                normalEdge += 1.0 - saturate(dot(n, nU));
                
                return saturate((depthDiff + normalEdge * 0.5) * 10.0);
            }
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                half outlineEdge = DetectOutlineEdge(input, screenUV);
                ...
                half3 finalColor = lerp(cloudColor, _OutlineColor.rgb, outlineEdge * _OutlineColor.a);
                half finalAlpha = max(alpha, outlineEdge * _OutlineColor.a);
