...
## 5. Статус 09R: Bake-тулза реализована

`Assets/_Project/Editor/FloatingOrigin/BakePilotSceneCatalog.cs`
(меню `ProjectC/World/Floating Origin/Bake Pilot Catalog (Preview + Apply)`)
выполняет §3.2 одной кнопкой: скан кандидатов → предложение treatment/ownership
→ превью diff → применение после вписанного reviewNote (аттестация человека,
без неё Apply заблокирован) → пересчёт хэшей/digest → ре-верификация.
Структурный дрейф (родитель/корень/сетевость) тулза не чинит — только руками.
Алгоритмы хэшей — из `AuditGlobalSceneCatalog` (открыты `public`, не дублировать).
`DryRun()` — headless read-only самопроверка (используется и в CI-подобных
проверках через MCP).
