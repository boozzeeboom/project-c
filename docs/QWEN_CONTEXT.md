...
**Ветка:** `qwen-dev` (основная) | **Этап:** Ранняя преальфа (прототип ядра)
...
|-----------|----------|------|--------------|
| 🔴 Критично | Disconnect кнопка в левом углу | `NetworkUI.cs:CreateDisconnectButton()` | Исправить якоря: `anchorMin/Max = (0.5, 0.5)`, `pivot = (0.5, 0.5)`, `anchoredPosition = Vector2.zero` |
| 🟡 Средне | Инвентарь не синхронизируется между игроками | Архитектура | Этап 3 (RPG система) |
...
- ❌ НЕ действую автономно — ВСЕГДА спрашиваю
- ❌ НЕ пропускаю чтение файла агента/навыка перед действием
- ❌ НЕ принимаю финальные решения за пользователя
- ❌ НЕ пишу файлы без явного одобрения
...
**Основные ветки:**
- `qwen-dev` — основная ветка разработки
- `qwen-gamestudio-agent-dev` — ветка для интеграции Game Studio
...
| 🔴 **Спрашивай перед записью** | "Могу записать в [filepath]?" |
