...
- Следующие обязательные части интеграции: trusted persistent identity mapping, authoritative global checkpoint collection/merge и native-ready restore для genuine source G; user acceptance native storage/backup/crash policy B, дальнейшая миграция ship/RPC DTO T-FO05. Authoring/baking source bindings, reviewed catalog, spatial/replacement/exclusion/DDOL/loading bridges T-FO06–08 остаются открыты, как и dirty/missing-component/uninspected scope H. I static/service subset и A/B persistence helpers НЕ завершают native gameplay, MMO streaming или distributed recovery. Prefab/scene activation, полный T-FO04/T-FO05 и T-FO06–09 не приняты. Runtime floating origin выключен, устранение микротряски не подтверждено.
