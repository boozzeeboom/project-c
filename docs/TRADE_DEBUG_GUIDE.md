...
| 0.3 | 9 апр 2026 | Hotfix: сервер всегда Load(), клиент Load() вместо Sync/Remove |
| 0.4 | 9 апр 2026 | **Сессия 8E**: Clamp факторов, валидация quantity/locationId, логи Save/Load |

---

## ⚠️ Известные проблемы (не исправлены)

### ~~demandFactor > 1.5~~ ✅ ИСПРАВЛЕНО

**Статус:** ✅ Исправлено в сессии 8E.

**Что сделано:**
- `MarketItem.RecalculatePrice()` — clamp факторов перед расчётом цены
- `LocationMarket.InitItems()` — clamp факторов при загрузке рынка
- Защита от corrupted ScriptableObject данных с demandFactor=20+

---

### ~~quantity <= 0~~ ✅ ИСПРАВЛЕНО

**Статус:** ✅ Исправлено в сессии 8E.

**Что сделано:**
- `BuyItemServerRpc` и `SellItemServerRpc` — валидация `quantity > 0`
- Защита от эксплойта: quantity < 0 давал бесконечные кредиты

---

### ~~locationId пустой~~ ✅ ИСПРАВЛЕНО

**Статус:** ✅ Исправлено в сессии 8E.

**Что сделано:**
- `BuyItemServerRpc` и `SellItemServerRpc` — валидация `locationId != null/empty`
- Защита от бесполезных RPC trips

---

### ~~Нет логов Save/Load~~ ✅ ИСПРАВЛЕНО

**Статус:** ✅ Исправлено в сессии 8E.

**Что сделано:**
- `PlayerTradeStorage.Save()` — лог с location, credits, items, weight, volume
- `PlayerTradeStorage.Load()` — лог до и после загрузки
- Упрощена отладка рассинхронизации склада

---

### PlayerPrefs для авторитетных данных (Отложено)

**Симптом:** При Dedicated Server + Client данные склада хранятся в PlayerPrefs разных процессов.

**Влияние:** В Host режиме работает (один процесс). В Dedicated Server — возможна рассинхронизация при переподключении.

**Решение:** Использовать серверное хранилище (БД или JSON файлы) вместо PlayerPrefs.

**Статус:** ⏳ Отложено на сессию 9+ (архитектурная задача).
