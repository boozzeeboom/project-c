...
|------|-----------|--------|
| Phase 0-1 | Инфраструктура: 9 языков, 4 таблицы, runtime-переключение | ✅ |
| Phase 2 | System-сообщения: 49 `sys.*` ключей, удаление серверных строк | ✅ |
| Phase 3 | EscMenu + KeybindingsWindow + 30 UI-ключей (character/contract/market/toast/dialog) | ✅ |
| Phase 4 | SO-мигратор: 119 ключей `Static_Table` (TradeItem 113, NPC 4, Quest 2) | ✅ |
| Phase 5 | Диалоги: 14 `dialogue.*` ключей + DialogWindow fixes | ✅ |
| Phase 6 | Инструмент переводчика: CSV Export/Import + валидация покрытия | ✅ |
| Phase 8 (LOC-11) | Глубокая UI-локализация: 87 новых ключей в 7 C# файлах | ✅ |
| T-LOC-12..17 | Финальная вычистка hardcode-строк | ✅ |
| T-LOC-ESC | Дропдаун языков в ESC-меню: адаптивный попап, ScrollView fix, синхронизация с `SettingsManager.Locale` (6 коммитов) | ✅ |

**Документация:** `docs/dev/ITERATIONS.md` (все фазы 0-6 + 9-10), `POSTMORTEM-LOC-10` (анализ ошибки CSV-экспорта).

---

### 1.22 Главное меню (Main Menu) ✅ v0.1.0 (T-UI04, 7 августа 2026)

**Цель:** Замена legacy `NetworkTestCanvas` на UI Toolkit главное меню с панелью настроек.

| Компонент | Назначение | Статус |
|-----------|-----------|--------|
| Главное меню UI Toolkit | Замена `NetworkTestCanvas`, полноценное меню | ✅ |
| Панель настроек | Вёрстка (фон, border, скрытие заголовков) | ✅ |
| Фикс открытия | Панель не добавлялась в визуальное дерево | ✅ |
