...
This is a complete QWENCODE agent architecture for game development. It
...
Templates are in `.QWENCODE/docs/templates/`:
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   - Configures QWENCODE.md, detects knowledge gaps, populates reference docs
   - Creates `.QWENCODE/docs/technical-preferences.md` with naming conventions,
...
```
QWENCODE.md                          -- Master config (read this first, ~60 lines)
.QWENCODE/
  settings.json                    -- QWENCODE hooks and project settings
