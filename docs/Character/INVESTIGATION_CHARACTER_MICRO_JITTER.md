...
> **Текущий этап — после T-JITTER18, 2026-09-08:** skinning-only эксперимент был подключён к настоящему `NetworkPlayer` и проверен пользователем на проблемной позиции около 56,5 км. Персонажа продолжает трясти; эксперимент не устраняет джиттер. Код восстановлен к baseline `57572dc006381f9523ed8d3c7d6a68e2baf7551f`, документация сохранена. Детали отрицательного результата: `docs/Character/IMPLEMENTATION_CHARACTER_LOCAL_SKINNING_EXPERIMENT.md`. Статусы ниже относятся к историческим этапам.
...
> **Актуализация 2026-09-08 — T-JITTER18 runtime result:** skinning-only эксперимент в реальной игре был проверен на проблемной дальней позиции примерно `(39877.62, 2502.17, 40026.44)` / около 56,5 км. Персонажа продолжает трясти, поэтому гипотеза local skinning proxy не подтверждена и код откатан к baseline `57572dc006381f9523ed8d3c7d6a68e2baf7551f`. Старые FloatingOrigin/FloatingOriginMP не используются. Широкая миграция мира из §9.5 **по-прежнему отложена**; текущий этап завершён отрицательным результатом, без автоматического перехода к новой архитектуре. Измерения и отрицательные результаты сохраняются как история.
...
## 1.1. Результат T-JITTER18

- Проверка выполнена в реальном Play Mode, а не в отдельном стенде.
- На проблемной дальней позиции около 56,5 км персонаж продолжил заметно трястись при включённом local-skinning proxy.
- Вывод: T-JITTER18 **не решает проблему микроджиттера**.
- Кодовый эксперимент удалён/восстановлен к baseline `57572dc006381f9523ed8d3c7d6a68e2baf7551f`; документация и история эксперимента сохранены.
...
> **Статус на 2026-09-08:** ниже сохранён исторический архитектурный вывод. Он НЕ является обязательным планом исправления текущего визуального бага. T-JITTER17 отделяет долгосрочную точность большого мира от ограниченного character-visual эксперимента; см. новый документ выше.
