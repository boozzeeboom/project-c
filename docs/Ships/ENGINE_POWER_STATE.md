...
> ⚠️ КРОСС-ЭФФЕКТ (T-SHIP-DOC06 2026-09-18): в состоянии `BROKEN`
> (`damage_subsystem/00_DESIGN.md §4`: скорости ×0.1, двигатель НЕ выключается)
> корабль проходит ту же дистанцию ~в 10 раз дольше → жрёт ~x10 топлива на дистанцию,
> затем гаснет по правилу ниже (`fuel==0` → авто-OFF → падение). Учитывать при балансе.
