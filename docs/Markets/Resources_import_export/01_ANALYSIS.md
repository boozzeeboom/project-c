...
CSV-импорт покрывает 4 из 4 (ItemData, TradeItemDefinition, MarketItemConfig, ExchangeRateEntry);
RecipeData — **Phase 2** (отдельная итерация).
...
11. **CSV-блок `recipes` (Phase 2, out of scope MVP):** `recipeName` уникален; все
    `ingredientItemName` и `outputItemName` есть в ItemRegistry. Эта проверка
    переедет в `CraftingCsvValidator` отдельной итерации.
...
- **Несколько блоков в одном CSV:** MVP — 4 блока (`inventory`, `tradeItems`, `marketItems`,
  `exchangeRates`). Phase 2 добавит 5-й блок `recipes`. Разделитель — пустая строка +
  строка-маркер `# block=<name>` (или секции в одном файле).
...
### ~~Блок 5: recipes (RecipeData)~~ — Phase 2 (out of scope MVP)
- `recipeName` (required, unique), `displayName`, `category`, `craftSeconds`,
  `requiredSkillLevel`, `requiredSkill`, `ingredient1Name`+`ingredient1Qty`, …
  `output1Name`+`output1Qty`, …
- **Phase 2:** парсер распознаёт секцию, importer выдаёт warning + skip. Реальный
  импорт — отдельный `CraftingCsvImporter` со своей схемой. Сейчас в MVP **не покрывается**;
  recipes управляются вручную в `Resources/Crafting/Recipes/`.

> **Итого (MVP): 4 блока в CSV.** Phase 2 добавит 5-й.
